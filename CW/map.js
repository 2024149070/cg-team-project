...
export const weatherZones = [
    {
        type: 'RAIN',
        position: { x: 34, y: 5, z: 0 },
        size: { x: 10, y: 10, z: 10 }
    },
    {
        type: 'SNOW',
        position: { x: 57, y: 5, z: 0 },
        size: { x: 10, y: 10, z: 10 }
    }
];
...
export async function initMap(loader, scene, colliders) {
    ...
    await traffic_light_zone(scene, loader, { x: 45, y: 0, z: 0 }, colliders);
    ...
    createRamp(scene, rampPositions, { length: 4, height: 2, width: 3 }, colliders);
...
const bigWallPosition = { x: 20, y: 2.5, z: 0 };
const bigWallPosition2 = { x: 90, y: -2.5, z: 0 };
...
// 경사로 생성.
const rampPositions = [{ x: 78, y: 0.25, z: 3 }];

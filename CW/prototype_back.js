...
import { floors, obstacles, goal, pillarPositions, pillars, steps,
    bigwalls, bigWallPositions, smallfloors, sinkHoles, conePositions } from './map.js';
...
////////// 블랙홀 관련 상태 변수 추가 //////////
// 남길 인덱스 지정 (랜덤x)
let sinkHoleStage = 'idle';
let sinkHoleShakingStart = 0;
const sinkHoleFirstX = Math.min(...sinkHoles.map(sh => sh.position.x));
const sinkHoleKeepIndices = [0, 8, 12, 17, 23, 27, 30, 33, 38];
const sinkHoleTriggerOffset = 5;
const sinkHoleShakeDuration = 700;
const sinkHoleShakeIntensity = 0.1;
...
sinkHoles.forEach(sh => {
    scene.add(sh);
    floors.push(sh);
    sh.userData.initialPosition = sh.position.clone();
});
scene.add(goal);


///// bigWall 추가 /////
const bigWallLoader = new GLTFLoader();
bigWallLoader.load(
    './assets/bigWall.glb',
    (gltf) => {
        ...
        bigWallPositions.forEach((pos, index) => {
            const bigWallMesh = template.clone(true);
            bigWallMesh.scale.set(0.7, 0.7, 0.7);
            bigWallMesh.position.set(pos.x, pos.y, pos.z);
            ...
            bigwalls.push(bigWallMesh);
            obstacles.push(bigWallMesh);
        });
    },
    undefined,
    (err) => {
...
///// 아파트 추가 /////
const apartLoader = new GLTFLoader();

apartLoader.load(
    './assets/apartment2.glb',
    (gltf) => {
        const template = gltf.scene;
...
function updatePhysics() {
    ...
    if (allPressed && bigwalls.length > TARGET_WALL_INDEX) {
        ...
        if (!targetWall.userData.raised) {
            ...
            targetWall.updateMatrixWorld(true);
    ...
    ///// 블랙홀 발생 /////
    // idle -> shaking
    if (sinkHoleStage === 'idle') {
        if (character.position.x >= sinkHoleFirstX - sinkHoleTriggerOffset) {
            sinkHoleStage = 'shaking';
            sinkHoleShakingStart = performance.now();
            console.log('싱크홀이 생겼습니다!');
        }
    }

    // shaking
    if (sinkHoleStage === 'shaking') {
        const time = performance.now() - sinkHoleShakingStart;
        const t = time / sinkHoleShakeDuration;

        if (t >= 1) {
            // 흔들림 끝 → 남기지 않을 타일은 떨어뜨리기
            sinkHoleStage = 'fallen';

            sinkHoles.forEach((sh, index) => {
                const keep = sinkHoleKeepIndices.includes(index);

                if (!keep) {
                    sh.position.y -= 1000;
                } else {
                    if (sh.userData.originalPosition) {
                        sh.position.copy(sh.userData.originalPosition);
                    }
                }
                sh.updateMatrixWorld(true);
                if (sh.geometry) {
                    sh.bbox = new THREE.Box3().setFromObject(sh);
                }
            });

        } else {
            // 흔들리는 상태
            const shakeOffset = Math.sin(t * Math.PI * 10) * sinkHoleShakeIntensity * (1 - t);

            sinkHoles.forEach((sh) => {
                if (!sh.userData.originalPosition) {
                    sh.userData.originalPosition = sh.position.clone();
                }
                sh.position.y = sh.userData.originalPosition.y + shakeOffset;
...
function resetGame() {
    ...
    sinkHoleStage = 'idle';
    sinkHoleShakingStart = 0;

    sinkHoles.forEach(sh => {
        if (sh.userData.initialPosition) {
            sh.position.copy(sh.userData.initialPosition);
            sh.updateMatrixWorld(true);
            if (sh.geometry) {
                sh.bbox = new THREE.Box3().setFromObject(sh);
            }
    ...
    bigwalls.forEach(wall => {
        if (wall.userData.initialPosition) {
            wall.position.copy(wall.userData.initialPosition);
        }
        wall.userData.raised = false;

...
export class Player {
    ...
    setMesh(mesh) {
        this.mesh = mesh;
        this.mesh.position.set(4, 1.5, 0);
    ...
    update(inputManager, isOrtho, obstacles, floors, steps, bigwalls) {
        if (!this.mesh) return;

        this.handleMovement(inputManager, isOrtho);
        this.applyGravity();
    ...
    handleMovement(inputManager, isOrtho) {
        const snowConfig = CONFIG.WEATHER.SNOW;
        const isOnSnow = this.mesh.position.x >= snowConfig.startX &&
            this.mesh.position.x <= snowConfig.endX;
        ...
        if (isOnSnow) {
            ...
            const isRainZone = this.mesh.position.x >= rainConfig.startX &&
                this.mesh.position.x <= rainConfig.endX;
    ...
    checkSteps(steps, bigwalls, isOrtho) {
        ...
        steps.forEach(step => {
            ...
            const onY = Math.abs(playerBottomY - max.y) < 0.1;

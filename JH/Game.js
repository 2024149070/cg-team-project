...
import { floors, obstacles, goal, steps, bigwalls, weatherZones, initMap, weatherFloors, weatherClouds, invisibleObstacles } from './map.js';
...
export class Game {
    ...
    async init() {
        try {
            ...
            await initMap(this.scene, this.assetManager);
    ...
    render() {
        ...
        if (!this.isGameOver) {
            const isDead = this.player.update(this.inputManager, this.cameraManager.isOrtho, obstacles, floors, steps, bigwalls, weatherZones);
            if (isDead) {
                this.isGameOver = true;
